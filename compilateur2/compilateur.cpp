...
string instructENCOURS; // Mémorisation de l'instruction en cours
...
void Program() {
    ...
    //**************************************************/
    //* Règles de syntaxe sous forme de grammaire EBNF */
    //++++++++++++++++++++++++++++++++++++++++++++++++++/
...
// La régle Statement est définie comme AssignementStatement | IfStatement | WhileStatement | ForStatement | BlockStatement
void Statement(){

    // Régle obligatoire d'entrée: présence obligatoire ID ou INSTRUCTS qui doit être IF ou WHILE ou FOR ou BEGIN
    if (current == INSTRUCTS) {
        ...
        if (!strcmp(lexer->YYText(), "IF")) {
            // Régle optionnelle: régle définie dans IfStatement()
            IfStatement();
        } else if (!strcmp(lexer->YYText(), "WHILE")) {
            // Régle optionnelle: régle définie dans WhileStatement()
            WhileStatement();
        } else if (!strcmp(lexer->YYText(), "FOR")) {
            // Régle optionnelle: régle définie dans ForStatement()
            ForStatement();
        } else if (!strcmp(lexer->YYText(), "BEGIN")) {
            // Régle optionnelle: régle définie dans BlockStatement()
            BlockStatement();
        }
    }else if(current == ID){
        // Régle optionnelle: régle définie dans AssignementStatement()
...
// Règle IfStatement définie comme: "IF" Expression "THEN" Statement [ "ELSE" Statement ]
void IfStatement(){

    // Vérification du jeton "IF"
    if (!strcmp(lexer->YYText(), "IF")){
    ...
    }else{Error("Un IF était attendu");}

}

// Règle WhileStatement définie comme: "WHILE" Expression "DO" Statement
void WhileStatement(){
    // Vérification du jeton "WHILE"
    if (!strcmp(lexer->YYText(), "WHILE")){

        // Instruction en cours
        instructENCOURS = "WHILE";
    ...
    }else{Error("Un WHILE était attendu");}
}

// Règle ForStatement définie comme: "FOR" AssignementStatement "To" Expression "DO"
void ForStatement(){
    // Vérification du jeton "FOR"
    if (!strcmp(lexer->YYText(), "FOR")){
        ...
        // Assembleur x86: creation etiquette pour le debut de la boucle for (Initialisation)
        cout << "PourInit" + to_string(compteurEtiquette) + ":" << endl;

        // Régle obligatoire: régle définie dans AssignementStatement();
        AssignementStatement();

        // Assembleur x86: creation etiquette pour le test de la boucle for (test)
        cout << "PourTest" + to_string(compteurEtiquette) + ":" << endl;
    ...
    }else{Error("Un FOR était attendu");}
}

// Règle BlockStatement définie comme: "BEGIN" Statement { ";" Statement } "END"
void BlockStatement(){
    // Vérification du jeton "BEGIN"
    if (!strcmp(lexer->YYText(), "BEGIN")) {

        // Instruction en cours
        instructENCOURS = "BEGIN";
        ...
        if (!strcmp(lexer->YYText(), "END")){
            ...
            // Lecture du jeton suivant
            current = (TOKEN) lexer->yylex();
        }else{Error("Un END était attendu");}
    ...
    if(current == ID){
        ...
        current=(TOKEN) lexer->yylex();
                    ...
                    if(current == ASSIGN) {
                        ...
                        // Autres registres utilisés %rcx et %rdx
                        if (instructENCOURS == "WHILE"){
                            // Assembleur x86: on récupère la valeur dans la pile pour l'assigner au registre %rax
                            cout << "\tpop %rax \t\t\t\t # AssignementStatement | topPile -> %rax" << endl;
                        }
                        // Initialisation de la boucle FOR
                        else if (instructENCOURS == "FOR"){
                            ...
                            // Assembleur x86: on copie la valeur assignée dans %rax
                            cout << "\tmovq " << variableAssignation << ", %rax \t\t\t\t # Number | " << variableAssignation << " -> %rax " << endl;
...
void Expression(){
    ...
    // Régle otionnelle:  le jeton doit être un RELOP  "==" ou "!=" ou "<" ou ">" ou "<=" ou ">=" avec une instruction FOR
    if(current == RELOP && instructENCOURS == "FOR" ) {
        // YYText() ne renvoie pas une chaîne de caractère de type string, mais un char *
        // On va donc utiliser la fonction strcmp() qui renvoie 0, donc False, quand les deux chaînes sont identiques.
        if(!strcmp(lexer->YYText(),"==")) {
            //Assembleur x86: saut si la condition suivante est valide:  %rax != 0
            variableRELOP = "\tjne finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax != 0 alors finPour" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"!=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax == 0
            variableRELOP = "\tje finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax == 0 alors finPour" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<")){
            //Assembleur x86: saut si la condition suivante est valide: %rax >= 0
            variableRELOP = "\tjae finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax >= 0 alors finPour" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax <= 0
            variableRELOP = "\tjbe finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax <= 0 alors finPour" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax > 0
            variableRELOP = "\tja finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax > 0 alors finPour" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax < 0
            variableRELOP = "\tjb finPour" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax < 0 alors finPour" + to_string(compteurEtiquette);
        }
        ...
        //Assembleur x86: on compare (sub) %rax avec la valeur assignée
        cout << "\tcmpq $" << lexer->YYText() << ", %rax \t\t\t\t # Expression | %rax - $" << lexer->YYText() <<  " -> %rax (comparer)" << endl;

        // Affichage de la relation mémorisée
        cout << variableRELOP << endl;

        // Lecture du jeton suivant (double sauts)
        current = (TOKEN) lexer->yylex();
        current = (TOKEN) lexer->yylex();
        //cout << lexer->YYText();
        ...
        //Assembleur x86: création Etiquette pour marquer le début des instructions après un DO
        cout << "intruction" + to_string(compteurEtiquette) + ":" << endl;

        // Régle obligatoire: régle définie dans Statement()
        Statement();

        //Assembleur x86: On ajoute $1 à %rax
        cout << "\taddq $1, %rax \t\t\t\t # Expression | %rax + 1 -> %rax (incrémentation)" << endl;

        //Assembleur x86: On saute à l'étiquette PourTest
        cout << "\tjmp PourTest"  + to_string(compteurEtiquette) + "\t\t\t\t # Expression | on saute l'étiquette PourTest"  + to_string(compteurEtiquette) << endl;

        //Assembleur x86: création Etiquette pour marquer la fin de la boucle FOR
        cout << "finPour" + to_string(compteurEtiquette) + ":" << endl;
    ...
    // Régle otionnelle:  le jeton doit être un RELOP  "==" ou "!=" ou "<" ou ">" ou "<=" ou ">=" avec une instruction WHILE
    if(current == RELOP && instructENCOURS == "WHILE" ) {

        // YYText() ne renvoie pas une chaîne de caractère de type string, mais un char *
        // On va donc utiliser la fonction strcmp() qui renvoie 0, donc False, quand les deux chaînes sont identiques.
        if(!strcmp(lexer->YYText(),"==")) {
            //Assembleur x86: saut si la condition suivante est valide:  %rax != 0
            variableRELOP = "\tjne finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax != 0 alors finTantQue" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"!=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax == 0
            variableRELOP = "\tje finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax == 0 alors finTantQuen" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<")){
            //Assembleur x86: saut si la condition suivante est valide: %rax >= 0
            variableRELOP = "\tjae finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax >= 0 alors finTantQue" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax <= 0
            variableRELOP = "\tjbe finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax <= 0 alors finTantQue" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax > 0
            variableRELOP = "\tja finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax > 0 alors finTantQue" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax < 0
            variableRELOP = "\tjb finTantQue" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax < 0 alors finTantQue" + to_string(compteurEtiquette);
        }
        ...
        //Assembleur x86: création étiquette unique pour le IF
        cout << "TanTQue" << compteurEtiquette << ":" << endl;
        ...
        // Cas tantque la condition est vraie: faire instructions
        ...
        //Assembleur x86: on saute directement à l'étiquette TantQue pour réaliser une boucle
        cout << "\tjmp TanTQue" << compteurEtiquette << "\t\t\t\t # Expression | on saute l'étiquette TantQue"  << compteurEtiquette << endl;


        //part 2
        //Assembleur x86: création étiquette unique pour la fin de l'instruction IF
        cout << "finTantQue" << compteurEtiquette << ":" << endl;
    ...
    // Régle otionnelle:  le jeton doit être un RELOP  "==" ou "!=" ou "<" ou ">" ou "<=" ou ">=" avec une instruction IF
    if(current == RELOP && instructENCOURS == "IF" ) {

        // YYText() ne renvoie pas une chaîne de caractère de type string, mais un char *
        // On va donc utiliser la fonction strcmp() qui renvoie 0, donc False, quand les deux chaînes sont identiques.
        if(!strcmp(lexer->YYText(),"==")) {
            //Assembleur x86: saut si la condition suivante est valide:  %rax != 0
            variableRELOP = "\tjne siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax != 0 alors siNon" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"!=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax == 0
            variableRELOP = "\tje siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax == 0 alors siNon" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax >= 0
            variableRELOP = "\tjae siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax >= 0 alors siNon" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax <= 0
            variableRELOP = "\tjbe siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax <= 0 alors siNon" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),"<=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax > 0
            variableRELOP = "\tja siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax > 0 alors siNon" + to_string(compteurEtiquette);
        }
        if(!strcmp(lexer->YYText(),">=")){
            //Assembleur x86: saut si la condition suivante est valide:  %rax < 0
            variableRELOP = "\tjb siNon" + to_string(compteurEtiquette) + "\t\t\t\t # Expression | si %rax < 0 alors siNon" + to_string(compteurEtiquette);
        }
        ...
        cout << "\tpop %rbx" << "\t\t\t\t # Expression | topPile -> %rbx" << endl;
        //Assembleur x86: création étiquette unique pour le IF
        cout << "debutIF" << compteurEtiquette << ":" << endl;
        //Assembleur x86: on compare (sub) %rax avec %rbx
        cout << "\tcmpq %rax, %rbx" << "\t\t\t\t # Expression | %rax - %rbx -> %rax (comparer)" << endl;
        ...
        cout << "alors" << compteurEtiquette << ":" << endl;
        // Cas vrai: instructions
        ...
        //Assembleur x86: on saute directement à l'étiquette fin de l'instruction IF
        cout << "\tjmp finIF" << compteurEtiquette << "\t\t\t\t # Expression | on saute l'étiquette FinIf"  << compteurEtiquette << endl;

        //part 2
        // Règle optionnelle: présence de ELSE avec Statement()
        ...
        //part 3
        //Assembleur x86: création étiquette unique pour la fin de l'instruction IF
        cout << "finIF" << compteurEtiquette << ":" << endl;
    ...
    if(current == RELOP && instructENCOURS == "" ){
...
void SimpleExpression(){
    ...
    while(current == ADDOP){
        ...
        if (instructENCOURS == "WHILE"){
            ...
            // On va donc utiliser la fonction strcmp() qui renvoie 0, donc False, quand les deux chaînes sont identiques.
            if (!strcmp(lexer->YYText(), "+")) {
                //Assembleur x86: ajoute %rcx à %rdx
                variableADDOP = "\taddq	%rcx, %rdx \t\t\t\t # SimpleExpression | %rdx + %rcx -> %rdx\n";
                //Assembleur x86: empile %rdx à la pile
                variableADDOP += "\tpush %rdx \t\t\t\t # SimpleExpression | %rdx -> topPile (Stockage résultat ADD)\n";
            }
            if (!strcmp(lexer->YYText(), "-")) {
                //Assembleur x86: ajoute -(%rcx) à %rdx
                variableADDOP = "\tsubq	%rcx, %rdx \t\t\t\t # SimpleExpression | %rdx - %rcx -> %rdx\n";
                //Assembleur x86: empile %rdx à la pile
                variableADDOP += "\tpush %rdx \t\t\t\t # SimpleExpression | %rdx -> topPile (Stockage résultat SUB)\n";
            }
            if (!strcmp(lexer->YYText(), "||")) {
                //Assembleur x86: ajoute %rcx à %rdx
                variableADDOP = "\taddq	%rcx, %rdx \t\t\t\t # SimpleExpression | %rdx + %rcx -> %rdx\n";
                //Assembleur x86: empile %rdx à la pile
                variableADDOP += "\tpush %rdx \t\t\t\t # SimpleExpression | %rdx -> topPile (Stockage résultat OR)\n";
            }
        ...
        // Autres registres utilisés %rcx et %rdx
        if (instructENCOURS == "WHILE"){
            //Assembleur x86: depile %rcx de la pile
            cout << "\tpop %rcx" << "\t\t\t\t # SimpleExpression | topPile -> %rcx" << endl;
            //Assembleur x86: depile %rdx de la pile
            cout << "\tpop %rdx"<< "\t\t\t\t # SimpleExpression | topPile -> %rdx" << endl;
...
void Term(){
    ...
    while(current == MULOP){
        ...
        if(!strcmp(lexer->YYText(),"%")){
            ...
            variableMULOP += "\tpush %rdx \t\t\t\t # Term | %rdx -> topPile (Stockage résultat MOD)\n";
        }
        // AND
        if(!strcmp(lexer->YYText(),"&&")){
    ...
    switch(current){
        ...
        case ID:
            ...
            if(instructENCOURS == "FOR") {
                ...
                cout << "\tpush " << lexer->YYText() << "\t\t\t\t # Factor | " << lexer->YYText() << " -> topPile"
                     << endl;
